...
private:
    ...
    bool start_power_helper() {
        ...
        // os._exit(0) on a write failure, not `break`/`sys.exit`: when the
        // parent goes away the pipe breaks, and a normal exit would still let
        // the interpreter flush stdout on the way out and print
...
// goes to sysvoltage_/syscurrent_/syspower_ and can therefore never reach
// power_for_device() and inflate an efficiency figure.
//
...
// Directory holding this executable, so the repo's own config/ can be found
// relative to the build tree rather than the current working directory.
...
// INA228 measures). Searched in order:
//   1. $MB_INA228_CONFIG
//   2. <repo>/config/ina228.conf  — the app's own config, versioned with it
//   3. $XDG_CONFIG_HOME/mb-benchmark-gui/ina228.conf (else ~/.config/…)
// Lines are "<port> = <label>[, <flag>...]", '#' starts a comment. No file found
// → empty map (probes fall back to "INA228#<n>").
//
...
inline std::map<std::string, RailSpec> load_label_map() {
    ...
    if (const char* e = std::getenv("MB_INA228_CONFIG")) {
        ...
        // The binary lives in <repo>/build, so config/ is one level up.
        const std::string exe = exe_dir();
        if (!exe.empty()) candidates.push_back(exe + "/../config/ina228.conf");

        std::string base;
        if (const char* x = std::getenv("XDG_CONFIG_HOME")) base = x;
        else if (const char* h = std::getenv("HOME")) base = std::string(h) + "/.config";
        if (!base.empty()) candidates.push_back(base + "/mb-benchmark-gui/ina228.conf");
    }
    std::string path;
    for (const auto& c : candidates) {
        std::ifstream probe(c);
        if (probe) { path = c; break; }
    }
...
void Probes::discover(std::vector<std::string>* notes) {
    ...
    }
    {
        ...
        // keeps the discovery order (and so the Power/Voltage/Current legend
        // order) reading board-level first, then per-rail.
...
void Probes::flatten() {
    ...
    // colour, the per-device aggregate, the CSV column prefix, and — the part
    // that is easy to miss — the Graphs/Accelerators filter, which matches on
    // device_name. An unfolded shunt metric would stay on screen after its card
    // was unticked.
    //
    // `suffix` is what distinguishes the four families once folded. They share
    // one CSV namespace, so "<accel> INA228" for all of them would collide:
    // POWER / TEMP / ENERGY / CHARGE.
    //
    // Power was the bare "<accel> INA228" until 2026-09-03, i.e. CSV column
    // `<bdf>_INA228`. Logs written before that still carry the old name, so
    // tools/csv-to-html-plot.py deliberately accepts both spellings.
    auto emit = [&](const std::vector<size_t>& order,
                    const std::vector<MetricInfo>& (DeviceProbe::*get)() const,
    ...
    emit(dev_order_, &DeviceProbe::temp_metrics, " TEMP", temp_metrics_);
    emit(dev_order_, &DeviceProbe::power_metrics, " POWER", power_metrics_);
    emit(dev_order_, &DeviceProbe::energy_metrics, " ENERGY", energy_metrics_);
    emit(dev_order_, &DeviceProbe::charge_metrics, " CHARGE", charge_metrics_);
    emit(dev_order_, &DeviceProbe::voltage_metrics, " VBUS", voltage_metrics_);
    emit(dev_order_, &DeviceProbe::current_metrics, " CURRENT", current_metrics_);
    emit(dev_order_, &DeviceProbe::syspower_metrics, " SYS POWER", syspower_metrics_);
    emit(dev_order_, &DeviceProbe::sysvoltage_metrics, " SYS VBUS", sysvoltage_metrics_);
    emit(dev_order_, &DeviceProbe::syscurrent_metrics, " SYS CURRENT", syscurrent_metrics_);
...
void Probes::poll() {
    ...
    auto fill = [&](const std::vector<size_t>& order,
                    const std::vector<double>& (DeviceProbe::*get)() const,
    ...
    fill(dev_order_, &DeviceProbe::temp_values, temp_values_);
    fill(dev_order_, &DeviceProbe::power_values, power_values_);
    fill(dev_order_, &DeviceProbe::energy_values, energy_values_);
    fill(dev_order_, &DeviceProbe::charge_values, charge_values_);
    fill(dev_order_, &DeviceProbe::voltage_values, voltage_values_);
    fill(dev_order_, &DeviceProbe::current_values, current_values_);
    fill(dev_order_, &DeviceProbe::syspower_values, syspower_values_);
    fill(dev_order_, &DeviceProbe::sysvoltage_values, sysvoltage_values_);
    fill(dev_order_, &DeviceProbe::syscurrent_values, syscurrent_values_);
    size_t fi = 0;
    for (auto& d : devices_)
...
double Probes::power_for_device(const std::string& device_name) const {
    // Same rule as the legend's per-device "max": a card may report several
    // power metrics (on-die POW plus a folded-on INA228 shunt) and the largest
    // is the most complete figure for the board. NaN when it reports none.
    double best = std::nan("");
    for (size_t i = 0; i < power_metrics_.size() && i < power_values_.size(); ++i) {
        if (power_metrics_[i].device_name != device_name) continue;
        const double v = power_values_[i];
